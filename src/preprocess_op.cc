...
void Permute::Run(cv::Mat* im, ImageBlob* data) {
    ...
    for (int i = 0; i < rc; ++i) {
        cv::extractChannel(*im, cv::Mat(rh, rw, CV_32FC1, base + i * rh * rw),
                           i);
    }
}
...
std::pair<float, float> Resize::GenerateScale(const cv::Mat& im) {
    ...
    if (keep_ratio_) {
        ...
        float scale_min = static_cast<float>(target_size_min) /
                          static_cast<float>(im_size_min);
        float scale_max = static_cast<float>(target_size_max) /
...
void LetterBoxResize::Run(cv::Mat* im, ImageBlob* data) {
    ...
    cv::resize(*im, *im, cv::Size(new_shape_w, new_shape_h), 0, 0,
               cv::INTER_AREA);
    ...
    cv::copyMakeBorder(*im, *im, top, bottom, left, right, cv::BORDER_CONSTANT,
                       cv::Scalar(127.5));
...
void GetAffineTrans(const cv::Point2f center, const cv::Point2f input_size,
                    const cv::Point2f output_size, cv::Mat* trans) {
    cv::Point2f srcTri[3];
    cv::Point2f dstTri[3];
...
void Pad::Run(cv::Mat* im, ImageBlob* data) {
    ...
    cv::copyMakeBorder(*im, *im, 0, h - rh, 0, w - rw, cv::BORDER_CONSTANT,
                       cv::Scalar(114));
    data->in_net_im_ = im->clone();
    data->in_net_shape_ = {
        static_cast<float>(im->rows),
...
const std::vector<std::string> Preprocessor::RUN_ORDER = {
    "InitInfo",   "TopDownEvalAffine", "Resize",    "LetterBoxResize",
    "WarpAffine", "NormalizeImage",    "PadStride", "Pad",
    "Permute"};
...
std::vector<cv::Mat> PadBatch(const std::vector<cv::Mat>& imgs) {
    ...
    for (int i = 0; i < imgs.size(); ++i) {
        cv::Mat im = imgs.at(i);
        cv::copyMakeBorder(im, im, 0, max_h - imgs.at(i).rows, 0,
                           max_w - imgs.at(i).cols, cv::BORDER_CONSTANT,
                           cv::Scalar(0));
