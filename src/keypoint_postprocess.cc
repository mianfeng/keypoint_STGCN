...
void transform_preds(std::vector<float>& coords, std::vector<float>& center,
    ...
    if (affine) {
        ...
        for (int p = 0; p < dim[1]; ++p) {
            affine_tranform(coords[p * 2], coords[p * 2 + 1], trans,
                            target_coords, p);
        }
    } else {
...
void get_max_preds(float* heatmap, std::vector<int>& dim,
    ...
    for (int j = 0; j < dim[1]; j++) {
        float* index = &(heatmap[batchid * num_joints * dim[2] * dim[3] +
                                 j * dim[2] * dim[3]]);
...
KeyPointResult PoseSmooth::smooth_process(KeyPointResult* result) {
    ...
    if (this->x_prev_hat.num_joints == -1) {
        ...
        std::fill(dx_prev_hat.keypoints.begin(), dx_prev_hat.keypoints.end(),
                  0.);
...
void PoseSmooth::PointSmooth(KeyPointResult* result,
                             ...
                             std::vector<float> thresholds, int index) {
    ...
    if (distance < thresholds[index] * this->thres_mult) {
        ...
        if (this->filter_type == "OneEuro") {
            keypoint_smoothed->keypoints[index * 3 + 1] = this->OneEuroFilter(
                result->keypoints[index * 3 + 1],
                this->x_prev_hat.keypoints[index * 3 + 1], index * 3 + 1);
            keypoint_smoothed->keypoints[index * 3 + 2] = this->OneEuroFilter(
                result->keypoints[index * 3 + 2],
                this->x_prev_hat.keypoints[index * 3 + 2], index * 3 + 2);
        } else {
            keypoint_smoothed->keypoints[index * 3 + 1] = this->ExpSmoothing(
                result->keypoints[index * 3 + 1],
                this->x_prev_hat.keypoints[index * 3 + 1], index * 3 + 1);
            keypoint_smoothed->keypoints[index * 3 + 2] = this->ExpSmoothing(
                result->keypoints[index * 3 + 2],
                this->x_prev_hat.keypoints[index * 3 + 2], index * 3 + 2);
        }
    }
