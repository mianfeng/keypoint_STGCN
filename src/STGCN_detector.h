...
struct CameraResult {
    int cameraId = -1;
    std::vector<FallResult> results;
    long timestamp = 0;
    cv::Mat imgs;
};

// Visualiztion reid Result
void VisualizeFallResult(const std::vector<Match>& match,
                         const CameraResult& results1,
                         const CameraResult& results2);
cv::Mat VisualizeFallResult(const cv::Mat& img, const CameraResult& results);
void RefineKeypointdata(std::vector<KeyPointResult>& results,
                        FallInput& inputs);
class FallDetector {
   public:
    explicit FallDetector(const std::string& model_dir,
    ...
    // Run predictor
    void Predict(std::vector<KeyPointResult>& results, FallInput& inputs);
   ...
   private:
    ...
    void Preprocess(std::vector<KeyPointResult>& results, ImageBlob* data,
                    FallInput& inputs);
    // Postprocess result
