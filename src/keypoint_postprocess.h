...
struct KeyPointResult {
    // Keypoints: shape(N x 3); N: number of Joints; 3: conf,x,y
...
class PoseSmooth {
   public:
    explicit PoseSmooth(const int width, const int height,
                        std::string filter_type = "OneEuro", float alpha = 0.5,
                        float fc_d = 0.1, float fc_min = 0.1, float beta = 0.1,
    ...
    void PointSmooth(KeyPointResult* result, KeyPointResult* keypoint_smoothed,
   ...
   private:
    ...
    std::vector<float> thresholds = {0.005, 0.005, 0.005, 0.005, 0.005, 0.01,
