...
cv::Mat VisualizeKptsResult(const cv::Mat& img,
                            ...
                            const std::vector<int>& colormap, int frame_ID) {
    const int edge[][2] = {{0, 1},   {0, 2},  {1, 3},   {2, 4},   {3, 5},
                           {4, 6},   {5, 7},  {6, 8},   {7, 9},   {8, 10},
                           {5, 11},  {6, 12}, {11, 13}, {12, 14}, {13, 15},
                           {14, 16}, {11, 12}};
    cv::Mat vis_img = img.clone();
    cv::Point pt1 = cv::Point(results[results.size() - 1].rect[0],
                              results[results.size() - 1].rect[1]);
    cv::Point pt2 = cv::Point(results[results.size() - 1].rect[2],
                              results[results.size() - 1].rect[3]);
    if (results[results.size() - 1].ID != 0) {
        cv::rectangle(vis_img, pt1, pt2, cv::Scalar(0, 255, 255), 2.5);
        if (results[results.size() - 1].ID == 2) {
            cv::Point origin;

            std::string id_text = "claping";
            origin.x = results[results.size() - 1].rect[2];
            origin.y = results[results.size() - 1].rect[1];
        ...
        std::string id_text = "FALL!!";
        origin.x = results[results.size() - 1].rect[0];
        origin.y = results[results.size() - 1].rect[1];
        cv::putText(vis_img, id_text, origin, cv::FONT_HERSHEY_PLAIN, 2,
                    cv::Scalar(0, 0, 255), 2);
        cv::rectangle(vis_img, pt1, pt2, cv::Scalar(0, 0, 255), 2.5);
    }
    for (int batchid = 0; batchid < 1; batchid++) {
        for (int i = 0; i < results[results.size() - 1].num_joints; i++) {
            if (results[results.size() - 1].keypoints[i * 3] > 0.5) {
                int x_coord =
                    int(results[results.size() - 1].keypoints[i * 3 + 1]);
                int y_coord =
                    int(results[results.size() - 1].keypoints[i * 3 + 2]);
                cv::circle(vis_img, cv::Point2d(x_coord, y_coord), 4,
                           cv::Scalar(0, 0, 255), 2);
            }
        }
        for (int i = 0; i < results[results.size() - 1].num_joints; i++) {
            int x_start =
                int(results[results.size() - 1].keypoints[edge[i][0] * 3 + 1]);
            int y_start =
                int(results[results.size() - 1].keypoints[edge[i][0] * 3 + 2]);
            int x_end =
                int(results[results.size() - 1].keypoints[edge[i][1] * 3 + 1]);
            int y_end =
                int(results[results.size() - 1].keypoints[edge[i][1] * 3 + 2]);
            cv::line(vis_img, cv::Point2d(x_start, y_start),
                     cv::Point2d(x_end, y_end), cv::Scalar(245, 222, 179), 2.5);
        }
    }
    std::string frameNum = "Frame: " + std::to_string(frame_ID);

    cv::putText(vis_img, frameNum, cv::Point(10, 30), cv::FONT_HERSHEY_PLAIN, 2,
...
void KeyPointDetector::Postprocess(std::vector<float>& output,
    ...
    for (int batchid = 0; batchid < output_shape[0]; batchid++) {
        get_final_preds(output, output_shape, idxout, idx_shape,
                        center_bs[batchid], scale_bs[batchid], preds, batchid,
                        this->use_dark);
        KeyPointResult result_item;
        result_item.rect = rect;
...
void KeyPointDetector::Predict(const std::vector<cv::Mat> imgs,
    ...
    for (int i = 0; i < repeats; i++) {
        ...
        auto out_tensor = predictor_->GetOutputHandle(output_names[0]);
        output_shape = out_tensor->shape();
        // for (auto out : output_shape) {
        //     SPDLOG_INFO("Output shape: {}", out);
        // }
        // Calculate output length
        int output_size = 1;
        for (int j = 0; j < output_shape.size(); ++j) {
    ...
    Postprocess(output_data_, output_shape, idx_data_, idx_shape, result, rect,
                center_bs, scale_bs);
