...
class KeyPointDetector {
   public:
    explicit KeyPointDetector(
        ...
        const std::string& run_mode = "paddle", const int batch_size = 1,
        const int gpu_id = 0, const int trt_min_shape = 1,
        const int trt_max_shape = 1280, const int trt_opt_shape = 640,
    ...
    void Predict(const std::vector<cv::Mat> imgs,
                 ...
                 std::vector<std::vector<float>>& scale, std::vector<int>& rect,
                 const double threshold = 0.5, const int warmup = 0,
                 const int repeats = 1,
